...
function hexToRgb(hex: string) {
  ...
  const parsed = parseInt(value, 16) || 0xffffff;
  return {
    b: (parsed & 255) / 255,
    g: ((parsed >> 8) & 255) / 255,
    r: ((parsed >> 16) & 255) / 255,
...
export default function LaserFlow({
  ...
  const lastSizeRef = useRef({ dpr: 0, height: 0, width: 0 });
  ...
  useEffect(() => {
    ...
    const setSizeNow = () => {
      ...
      const sizeChanged =
        Math.abs(width - lastSize.width) > 0.5 || Math.abs(height - lastSize.height) > 0.5;
      const dprChanged = Math.abs(pixelRatio - lastSize.dpr) > 0.01;

      if (!sizeChanged && !dprChanged) return;

      lastSizeRef.current = { dpr: pixelRatio, height, width };
    ...
    const updateMouse = (clientX: number, clientY: number) => {
      ...
      mouseTarget.set(x * ratio, height - y * ratio);
    };
    ...
    canvas.addEventListener("pointerenter", onPointerMove, { passive: true });
    ...
    const adjustDprIfNeeded = (now: number) => {
      ...
      } else if (averageFps > upperThreshold && currentDprRef.current < base) {
        next = clamp(currentDprRef.current * 1.1, dprFloor, base);
      }

      if (Math.abs(next - currentDprRef.current) > 0.01 && now - lastDprChangeAt > cooldown) {
    ...
    const animate = () => {
      ...
      emaDtRef.current = emaDtRef.current * 0.9 + deltaMs * 0.1;
      fpsSamplesRef.current.push(1000 / Math.max(1, emaDtRef.current));

      uniforms.iTime.value = time;

      const clampedDt = Math.min(0.033, Math.max(0.001, delta));
      uniforms.uFlowTime.value = (uniforms.uFlowTime.value as number) + clampedDt;
  ...
  }, [color, decay, dpr, falloffStart, flowSpeed, flowStrength, fogFallSpeed, fogIntensity, fogScale, horizontalBeamOffset, horizontalSizing, mouseSmoothTime, mouseTiltStrength, verticalBeamOffset, verticalSizing, wispDensity, wispIntensity, wispSpeed]);
  ...
  useEffect(() => {
    ...
    const { r, g, b } = hexToRgb(color);
    (uniforms.uColor.value as THREE.Vector3).set(r, g, b);
  }, [color, decay, falloffStart, flowSpeed, flowStrength, fogFallSpeed, fogIntensity, fogScale, horizontalBeamOffset, horizontalSizing, mouseTiltStrength, verticalBeamOffset, verticalSizing, wispDensity, wispIntensity, wispSpeed]);
